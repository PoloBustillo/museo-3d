    ...
    <Dialog open={open} onOpenChange={v => { if (!v) handleClose(); }}>
      ...
      <div style={{ marginTop: 18, textAlign: 'center', fontSize: 15, background:'#f5f5fa', borderRadius:8, padding:'0.7em 0', color:'#222', fontWeight:500 }}>
        {localMode === "login" ? (
          <>
            ¿No tienes cuenta?{' '}
            <a href="#" onClick={e => { e.preventDefault(); setLocalMode('register'); }} style={{ color: '#3949ab', fontWeight: 'bold', textDecoration: 'underline', cursor: 'pointer', background:'#fff', padding:'2px 8px', borderRadius:4 }}>Crear una</a>
          </>
        ) : (
          <>
            ¿Ya tienes cuenta?{' '}
            <a href="#" onClick={e => { e.preventDefault(); setLocalMode('login'); }} style={{ color: '#3949ab', fontWeight: 'bold', textDecoration: 'underline', cursor: 'pointer', background:'#fff', padding:'2px 8px', borderRadius:4 }}>Iniciar sesión</a>
          </>
        )}
      </div>
    </Dialog>
