...
export default function CrearSala() {
  ...
  const handleAddMural = () => {
    if (selectedMuralForAdd && !selectedMurales.includes(parseInt(selectedMuralForAdd))) {
  ...
  return (
    <div className="relative min-h-screen">
      ...
      <div 
        className={`gallery-grid relative z-10 w-full max-w-6xl mx-auto px-4 sm:px-8 pt-24 md:pt-28 pb-8 md:pb-12 ${hasActiveGlow ? 'has-active-glow' : ''}`}
      ...
      >
              ...
              <div>
                ...
                <div className="flex gap-2 mb-3">
                  <div className="flex-1">
                    ...
                    >
                      <SelectContent>
                        {murales.filter(m => !selectedMurales.includes(m.id)).map((mural) => (
                          <SelectItem key={mural.id} value={mural.id.toString()}>
                            {mural.titulo}
                          </SelectItem>
                        ))}
                      </SelectContent>
                ...
                {selectedMurales.length > 0 && (
                  <div className="flex flex-wrap gap-2">
                    {selectedMurales.map((muralId) => {
                      ...
                      return mural ? (
                        <Badge key={muralId} variant="blue" className="flex items-center gap-1 px-3 py-1">
                          {mural.titulo}
                          <button
                            type="button"
        ...
        {view === "mis-salas" && (
          ...
          >
            <div className="gallery-card-glow bg-card rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300 border border-border p-8">
              <h2 className="text-2xl font-bold text-foreground mb-6">Mis salas</h2>
              {loadingSalas ? (
        ...
        <div className="mt-12 flex gap-4 justify-center">
          ...
          <div className="mt-12 flex gap-4 justify-center">
            <button
              className="px-6 py-3 rounded-xl bg-white/80 dark:bg-neutral-800/80 text-indigo-900 dark:text-white font-bold shadow-lg hover:bg-white dark:hover:bg-neutral-700 hover:shadow-xl transition-all duration-200 transform hover:scale-105"
              onClick={() => setView("crear")}
            >
              Crear nueva sala
            </button>
            <button
              className="px-6 py-3 rounded-xl bg-gradient-to-r from-indigo-700 to-indigo-500 text-white font-bold shadow-lg hover:from-indigo-800 hover:to-indigo-600 hover:shadow-xl transition-all duration-200 transform hover:scale-105"
              onClick={() => setView("mis-salas")}
            >
              Ver y editar mis salas
            </button>
