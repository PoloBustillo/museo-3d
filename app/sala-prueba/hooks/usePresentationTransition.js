...
// State machine: idle -> easingOut -> flyingIn -> exploring
// Provides unified interface for presentation flow.
export function usePresentationTransition({ easeOutMs = 600, onExplore }) {
  ...
  const registerBegin = useCallback(fn => {
    ...
    if (pendingStartRef.current && state === 'easingOut') {
      // If user pressed start early and we are waiting, attempt immediately
  ...
  pendingStartRef.current = true;
    ...
    const attempt = () => {
      if (beginRef.current) {
        console.log('[PresentationTransition] begin() triggered');
        beginRef.current();
        setState('flyingIn');
    pendingStartRef.current = false;
        ...
        console.error('[PresentationTransition] Failed to start entrance animation (begin missing). Forcing explore.');
        setState('exploring');
        onExplore && onExplore();
    pendingStartRef.current = false;
      }
