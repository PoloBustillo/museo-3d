...
function getMockSalaData() {
  return {
    ...
    murales: [
      { mural: { id: 1, titulo: 'Paisaje Urbano Nocturno', autor: 'Elena Martínez', anio: 2023, tecnica: 'Óleo sobre lienzo', descripcion: 'Una visión contemporánea de la ciudad en la noche', imagenUrlWebp: '/images/placeholder-artwork-1.jpg', url_imagen: '/images/placeholder-artwork-1.jpg' }, scale: 1.2, frameStyle: 'classic' },
      { mural: { id: 2, titulo: 'Reflexiones en Azul', autor: 'Carlos Mendoza', anio: 2024, tecnica: 'Fotografía digital', descripcion: 'Serie fotográfica sobre la luz y el agua', imagenUrlWebp: '/images/placeholder-artwork-2.jpg', url_imagen: '/images/placeholder-artwork-2.jpg' }, scale: 1.0, frameStyle: 'modern' },
      { mural: { id: 3, titulo: 'Memoria Colectiva', autor: 'Ana Ruiz', anio: 2023, tecnica: 'Técnica mixta', descripcion: 'Exploración de la identidad a través de materiales diversos', imagenUrlWebp: '/images/placeholder-artwork-3.jpg', url_imagen: '/images/placeholder-artwork-3.jpg' }, scale: 1.5, frameStyle: 'ornate' },
      { mural: { id: 4, titulo: 'Geometrías del Tiempo', autor: 'Roberto Silva', anio: 2024, tecnica: 'Acrílico sobre madera', descripcion: 'Abstracción geométrica inspirada en el paso del tiempo', imagenUrlWebp: '/images/placeholder-artwork-4.jpg', url_imagen: '/images/placeholder-artwork-4.jpg' }, scale: 0.8, frameStyle: 'minimal' }
    ],
    _count: { murales: 4 }
  };
}
...
const classifyAnchors = (points) => {
  ...
  for (const a of points) {
    ...
    switch (a.wall) {
      case 'right-front': sections.rf.push(a); break;
      case 'left-front': sections.lf.push(a); break;
      case 'right-mid': sections.rm.push(a); break;
      case 'left-mid': sections.lm.push(a); break;
      case 'right-back': sections.rb.push(a); break;
      case 'left-back': sections.lb.push(a); break;
      case 'back': sections.bw.push(a); break;
      default: break; // ignorar otros
    }
  }
...
export function useSalaData(salaId = null) {
  ...
  const fetchFirstAvailableSala = useCallback(async () => {
    ...
    try {
      ...
      let picked = salas.find(s => s.nombre === 'Colección ARPA' || s.id === 1 && s.murales?.length);
      if (!picked) picked = salas.find(s => s.murales && s.murales.length > 0);
      ...
      setLoading(false);
      if (DEBUG_SALA) console.log(`⏱️ fetchFirstAvailableSala ${(performance.now()-started).toFixed(1)}ms`);
    }
  }, []);
  ...
  const fetchSalaById = useCallback(async (id) => {
    ...
    const started = performance.now();
    try {
      const response = await fetch(`/api/salas/${id}`, { signal: controller.signal });
      ...
      setLoading(false);
      if (DEBUG_SALA) console.log(`⏱️ fetchSalaById ${(performance.now()-started).toFixed(1)}ms`);
    }
  }, []);

  // Efecto inicial / cambio de salaId
  useEffect(() => {
    if (!salaId) fetchFirstAvailableSala(); else fetchSalaById(salaId);
  ...
  const artworksRaw = useMemo(() => {
    ...
    if (DEBUG_ALLOC) console.log(`🧪 convertMuralesToArtworks: ${(performance.now()-start).toFixed(2)}ms`);
  ...
  const allocateAnchors = useCallback((arts) => {
    ...
    const willOverlap = (art, anchor, placed) => {
      ...
      const axisIndex = (wall.includes('back') || wall.includes('divider')) ? 0 : 2; // 0->X, 2->Z
      const span = (wall.includes('back') || wall.includes('divider')) ? (art.width||6) : (art.width||6); // width usado en ambos casos como extensión visible
      const half = span * 0.5;
    ...
    for (const art of sorted.slice(0,target)) {
      ...
      if (chosenAnchor) {
        ...
        if (DEBUG_ALLOC) console.warn('⚠️ No se encontró anchor sin solape para obra', art.id);
      }
    }
    ...
    const result = arts.map(a => ({ ...a, anchorId: assignments.get(a.id) || null }));
    if (DEBUG_ALLOC) console.log(`🧩 allocateAnchors (no-overlap) ${(performance.now()-t0).toFixed(2)}ms placed=${placed.length}/${arts.length}`);
    return result;
  }, []);
  ...
  return {
    ...
    loading,
    error,
    refresh: () => { salaId ? fetchSalaById(salaId) : fetchFirstAvailableSala(); }
  };
}
