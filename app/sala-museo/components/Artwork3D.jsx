 ...
 useEffect(()=>{ const id=setInterval(()=>{ if(!groupRef.current) return; const worldPos=tmpWorldPosRef.current; groupRef.current.getWorldPosition(worldPos); const toArtwork=tmpToArtworkRef.current.copy(worldPos).sub(camera.position).normalize(); camera.getWorldDirection(tmpCamDirRef.current); const facing=tmpCamDirRef.current.dot(toArtwork); const dist=camera.position.distanceTo(worldPos); const isFocused= facing>0.985 && dist<40; setFocused(p=> p!==isFocused? isFocused:p); focusedRef.current=isFocused; },300); return ()=> clearInterval(id); },[camera]); useEffect(()=>{ if(lampSpotRef.current && lampTargetRef.current){ lampSpotRef.current.target=lampTargetRef.current; lampSpotRef.current.target.updateMatrixWorld(); } },[dims.h,dims.w]); const handlePointerOver=()=> interactive && setHovered(true); const handlePointerOut=()=> interactive && setHovered(false); const handleClick=(e)=>{ e.stopPropagation(); if(artwork) openModal('artwork-modal',{ artwork:{ ...artwork } }); };
  return (<group ref={groupRef} scale={dims.w>width? width/dims.w:1} onPointerOver={handlePointerOver} onPointerOut={handlePointerOut} onClick={handleClick} dispose={null}>
    ...
    <group position={[0,-4.8,1.8]}>
      ...
      {/* Aiming housing tilted back toward wall */}
      <group position={[0,0.12,0]} rotation={[-Math.PI/5,0,0]}>
        <mesh castShadow>
        ...
        <mesh position={[0,0,0.10]}>
          ...
          <meshStandardMaterial color="#ffffff" emissive="#ffc674" emissiveIntensity={0.65} roughness={0.22} metalness={0.12} />
        </mesh>
      </group>
      <mesh ref={floorBulbRef} position={[0,0.14,0.04]}>
        <sphereGeometry args={[0.09,32,32]} />
        <meshStandardMaterial emissive={'#ffac33'} emissiveIntensity={3.0} color="#ffb347" />
      </mesh>
      {/* Hidden target near lower half of artwork (toward wall behind artwork) */}
      <mesh ref={(el)=>{ if(el && floorSpotMainRef.current){ floorSpotMainRef.current.target = el; floorSpotMainRef.current.target.updateMatrixWorld(); } if(el && floorHaloRef.current){ floorHaloRef.current.target = el; floorHaloRef.current.target.updateMatrixWorld(); } }} position={[0, (dims.h*0.25), 0.15]} visible={false} />
      <spotLight ref={floorSpotMainRef} position={[0,0.14,0.06]} angle={Math.PI/11} penumbra={0.9} intensity={10.2} distance={13.5} decay={1.5} color={'#ff9d2d'} castShadow={false} />
      <spotLight ref={floorHaloRef} position={[0,0.14,0.06]} angle={Math.PI/5} penumbra={1} intensity={2.9} distance={10} decay={1.7} color={'#ffb85c'} castShadow={false} />
      <pointLight ref={floorFillRef} position={[0,0.09,0]} intensity={0.75} distance={5.5} decay={2} color={'#ffac33'} />
    </group>
  </group> ); });
