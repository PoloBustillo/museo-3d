 ...
 */
export function RoomSelectorModal({ isOpen, onClose, onSelectRoom, availableRooms = [], currentRoom }) {
  const [selectedIndex, setSelectedIndex] = useState(0);
  const [selectedType, setSelectedType] = useState('room'); // 'room' o 'cancel'

  // Filtrar salas disponibles (excluyendo la actual)
  const selectableRooms = availableRooms.filter(room => room.id !== currentRoom);
  ...
  }, [isOpen, selectedIndex, selectedType, selectableRooms, totalOptions, onClose]);
          ...
          >
                ...
                {selectableRooms.map((room, index) => (
                  <motion.div
                    ...
                    style={{
                      padding: '1.2rem',
                      borderRadius: '12px',
                      border: selectedType === 'room' && selectedIndex === index ? '3px solid #4caf50' : '2px solid #e0e0e0',
                      backgroundColor: selectedType === 'room' && selectedIndex === index ? '#e8f5e8' : '#f9f9f9',
                      cursor: 'pointer',
                      display: 'flex',
                      alignItems: 'center',
                      gap: '1rem',
                      transition: 'all 0.3s ease',
                      outline: selectedType === 'room' && selectedIndex === index ? '2px solid #4caf50' : 'none',
                      outlineOffset: '2px'
                    }}
            ...
            <div style={{ 
              ...
              <button
                ...
                style={{
                  padding: '0.8rem 2rem',
                  borderRadius: '10px',
                  border: selectedType === 'cancel' ? '3px solid #4caf50' : '2px solid #666',
                  backgroundColor: selectedType === 'cancel' ? '#e8f5e8' : 'transparent',
                  color: selectedType === 'cancel' ? '#4caf50' : '#666',
                  fontSize: '1rem',
                  fontWeight: 'bold',
                  cursor: 'pointer',
                  transition: 'all 0.3s ease',
                  outline: selectedType === 'cancel' ? '2px solid #4caf50' : 'none',
                  outlineOffset: '2px'
                }}
              >
            ...
            <div style={{
              ...
              <p style={{ 
                ...
                🎮 <strong>Controles:</strong> ↑↓ para navegar • Enter para seleccionar • Esc para cerrar
              </p>
            </div>
          </motion.div>
        </motion.div>
