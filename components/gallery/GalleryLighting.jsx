 ...
 */
export function GalleryLighting({ dynamicLength, dynamicCenterX, lightingPreset, ambientIntensity }) {
  const numPointLights = Math.max(2, Math.floor(dynamicLength / 6));
  const numLamps = Math.floor(dynamicLength / 8);

  const presetFactory = LIGHTING_PRESETS[lightingPreset] || LIGHTING_PRESETS.neutral;
      ...
      {Array.from({ length: numLamps }).map((_, i) => (
        <React.Fragment key={`lamp-group-${i}`}>
          ...
          <mesh position={[
            ...
            <meshStandardMaterial color={lightingPreset === 'dramatic' ? '#444' : '#f8bbd0'} />
          </mesh>
        </React.Fragment>
      ))}
    </>
