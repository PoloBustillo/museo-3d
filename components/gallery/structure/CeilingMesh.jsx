  ...
  return (
    <group>
      ...
      <mesh
        ...
        castShadow
      >
        <boxGeometry args={[dynamicLength + 0.2, CEILING_THICKNESS, hallWidth + floorExtra + 0.2]} />
        {ceilingMaterial}
      </mesh>

      {/* Cornisas perimetrales profesionales */}
      {/* Cornisa frontal */}
      <mesh position={[dynamicCenterX, ceilingHeight + CEILING_THICKNESS + CORNICE_HEIGHT/2, (hallWidth + floorExtra)/2 + CORNICE_DEPTH/2]} receiveShadow castShadow>
        <boxGeometry args={[dynamicLength + 0.4, CORNICE_HEIGHT, CORNICE_DEPTH]} />
        <meshStandardMaterial color={CORNICE_COLOR} roughness={CORNICE_ROUGH} metalness={CORNICE_METAL} />
      </mesh>
      {/* Cornisa trasera */}
      <mesh position={[dynamicCenterX, ceilingHeight + CEILING_THICKNESS + CORNICE_HEIGHT/2, -(hallWidth + floorExtra)/2 - CORNICE_DEPTH/2]} receiveShadow castShadow>
        <boxGeometry args={[dynamicLength + 0.4, CORNICE_HEIGHT, CORNICE_DEPTH]} />
      ...
      {/* Cornisa izquierda */}
      <mesh position={[dynamicCenterX - dynamicLength/2 - CORNICE_DEPTH/2, ceilingHeight + CEILING_THICKNESS + CORNICE_HEIGHT/2, 0]} receiveShadow castShadow>
        <boxGeometry args={[CORNICE_DEPTH, CORNICE_HEIGHT, hallWidth + floorExtra + 2*CORNICE_DEPTH]} />
        <meshStandardMaterial color={CORNICE_COLOR} roughness={CORNICE_ROUGH} metalness={CORNICE_METAL} />
      </mesh>
      {/* Cornisa derecha */}
      <mesh position={[dynamicCenterX + dynamicLength/2 + CORNICE_DEPTH/2, ceilingHeight + CEILING_THICKNESS + CORNICE_HEIGHT/2, 0]} receiveShadow castShadow>
      ...
      {Array.from({ length: coffersX }).map((_, i) =>
        Array.from({ length: coffersZ }).map((_, j) => {
          const x = dynamicCenterX - dynamicLength/2 + (i + 0.5) * (dynamicLength / coffersX);
          const z = -(hallWidth + floorExtra)/2 + (j + 0.5) * ((hallWidth + floorExtra) / coffersZ);
          return (
            <mesh
              key={`coffer-${i}-${j}`}
              ...
              receiveShadow
            >
              <boxGeometry args={[COFFER_WIDTH * 0.85, COFFER_DEPTH, COFFER_LENGTH * 0.85]} />
              <meshStandardMaterial color={PANEL_COLOR} roughness={PANEL_ROUGH} metalness={PANEL_METAL} />
            </mesh>
          );
        })
      )}
      ...
      {Array.from({ length: coffersX + 1 }).map((_, i) => {
        const x = dynamicCenterX - dynamicLength/2 + i * (dynamicLength / coffersX);
        return (
          <mesh
            key={`beam-x-${i}`}
          ...
          >
            ...
            <meshStandardMaterial color={CORNICE_COLOR} roughness={0.3} metalness={0.2} />
          </mesh>
        );
      })}
      ...
      {Array.from({ length: coffersZ + 1 }).map((_, j) => {
        const z = -(hallWidth + floorExtra)/2 + j * ((hallWidth + floorExtra) / coffersZ);
        return (
          <mesh
            key={`beam-z-${j}`}
      ...
      <mesh position={[dynamicCenterX, ceilingHeight + CEILING_THICKNESS + CORNICE_HEIGHT + 0.06, 0]} receiveShadow castShadow>
        <boxGeometry args={[0.15, 0.08, hallWidth + floorExtra]} />
        <meshStandardMaterial color={ACCENT_COLOR} metalness={ACCENT_METAL} roughness={ACCENT_ROUGH} />
      </mesh>
      ...
      {[
        ...
        <mesh key={`corner-${idx}`} position={[x, ceilingHeight + CEILING_THICKNESS + CORNICE_HEIGHT + 0.08, z]} receiveShadow castShadow>
