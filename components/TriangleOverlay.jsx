...
export default function AnimatedTriangleOverlay({
  ...
  const buttonOpacity = isFinalStep ? Math.max(0, (appearProgress - 0.4) / 0.6) : 0; // Botón aparece al final
  ...
  return (
    ...
    >
      <div 
        className={`
          max-w-xs sm:max-w-sm md:max-w-md lg:max-w-lg transition-all duration-500 ease-out px-4 sm:px-6 md:px-8
          ${isLeft ? 'safe-pad-left text-left' : 'ml-auto safe-pad-right text-right'}
          ${isVisible && scrollOpacity > 0.5 ? 'pointer-events-auto' : 'pointer-events-none'}
        `}
        style={{
