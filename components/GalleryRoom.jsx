...
import { getPersonalCollection, isInPersonalCollection } from "../lib/personalCollection.js";
...
function Picture({ src, title, artist, year, description, technique, dimensions, position, rotation = [0,0,0], onClick, showPlaque, selected, selectedArtwork, spotlightIntensity=1, frameStyle, floorTextureUrl }) {
  const texture = useTexture(src);
  const resolvedFramePath = floorTextureUrl && !floorTextureUrl.includes("/") ? FLOOR_TEXTURE_ALIAS[floorTextureUrl] : floorTextureUrl;
  ...
  if (frameTexture) {
    ...
    frameTexture.repeat.set(1,1); frameTexture.anisotropy = 8;
  }
  const [hovered,setHovered] = useState(false);
  const [imageDimensions,setImageDimensions] = useState({ width:3, height:2 });
  useEffect(()=>{ const img=new Image(); img.onload=()=>{ if(img.width>0&&img.height>0){ const ar=img.width/img.height; const maxW=4,maxH=3; let w=maxW,h=maxW/ar; if(h>maxH){ h=maxH; w=maxH*ar;} if(isFinite(w)&&isFinite(h)) setImageDimensions({width:w,height:h}); } }; img.onerror=()=>{ setImageDimensions({width:3,height:2}); }; img.crossOrigin="anonymous"; img.src=src; },[src]);
  ...
  return (<group position={position} rotation={rotation} scale={scale} onPointerOver={()=>setHovered(true)} onPointerOut={()=>setHovered(false)}>
    ...
    <mesh position={[0,-h/2-thickness/2,depth]}><boxGeometry args={[w+thickness*2,thickness,thickness]}/><meshStandardMaterial map={frameTexture||null} color={!frameTexture?frameColor:undefined} metalness={0.4} roughness={0.5}/></mesh>
    <mesh position={[-w/2-thickness/2,0,depth]}><boxGeometry args={[thickness,h+thickness*2,thickness]}/><meshStandardMaterial map={frameTexture||null} color={!frameTexture?frameColor:undefined} metalness={0.4} roughness={0.5}/></mesh>
    <mesh position={[w/2+thickness/2,0,depth]}><boxGeometry args={[thickness,h+thickness*2,thickness]}/><meshStandardMaterial map={frameTexture||null} color={!frameTexture?frameColor:undefined} metalness={0.4} roughness={0.5}/></mesh>
    <mesh position={[0,0,0]} onClick={(e)=>{ e.stopPropagation(); onClick({ src,title,artist,year,description,technique,dimensions}); }}>
...
function PlayerControls({ onPassInitialWall, FIRST_X, LAST_X, WALL_MARGIN_INITIAL, WALL_MARGIN_FINAL, onReachEnd }) {
  const passedWallRef = useRef(false); const { camera } = useThree();
...
function SceneFog({ fog }) { const { scene } = useThree(); useEffect(()=>{ if(fog){ scene.fog = new THREE.Fog(fog.color||"#ffffff", fog.near??0, fog.far??50);} else scene.fog=null; return ()=>{ scene.fog=null; }; },[fog,scene]); return null; }

function ManualLookControls(){ const { camera, gl } = useThree(); const looking=useRef(false); const sens=0.0025; useEffect(()=>{ const el=gl.domElement; const onContext=e=>e.preventDefault(); const onDown=e=>{ if(e.button===2){ looking.current=true; el.requestPointerLock&&el.requestPointerLock(); } }; const onUp=e=>{ if(e.button===2){ looking.current=false; if(document.pointerLockElement) document.exitPointerLock&&document.exitPointerLock(); } }; const onMove=e=>{ if(!looking.current) return; const dx=e.movementX||0; const dy=e.movementY||0; camera.rotation.order="YXZ"; camera.rotation.y -= dx*sens; camera.rotation.x -= dy*sens; const maxPitch=Math.PI/2 - 0.05; camera.rotation.x=Math.max(-maxPitch,Math.min(maxPitch,camera.rotation.x)); }; el.addEventListener("contextmenu",onContext); el.addEventListener("mousedown",onDown); window.addEventListener("mouseup",onUp); window.addEventListener("mousemove",onMove); return ()=>{ el.removeEventListener("contextmenu",onContext); el.removeEventListener("mousedown",onDown); window.removeEventListener("mouseup",onUp); window.removeEventListener("mousemove",onMove); if(document.pointerLockElement) document.exitPointerLock&&document.exitPointerLock(); }; },[camera,gl]); return null; }

function Room({ artworks, galleryDimensions, passedInitialWall, setSelectedArtwork, selectedArtwork, showList, showCollection, showInstructions, layoutItems, scene, salaTextures }) {
  ...
  const autoFromLayout = useMemo(()=>{ if(!layoutItems||!layoutItems.length) return []; if(!layoutAllCenteredX) return []; const dummy = layoutItems.map((li,i)=>({ id: li.mural?.id||li.muralId||i, titulo: li.mural?.titulo||"Obra", url_imagen: li.mural?.url_imagen||li.mural?.imagenUrlWebp })); const dims=calculateGalleryDimensions(dummy); return calculateArtworkPositions(dummy,dims.firstX,GALLERY_CONFIG.PICTURE_SPACING,dims.contentLength).map((p,i)=>({ ...layoutItems[i], pos:{ ...(layoutItems[i].pos||{}), x:p.position[0], y:layoutItems[i].pos?.y ?? 1.5, z:p.position[2]>0?WALL_Z:-WALL_Z }, rot:{ ...(layoutItems[i].rot||{}), y:p.rotation[1] } })); },[layoutItems,layoutAllCenteredX]);
  const normalizedLayout = useMemo(()=>{ if(!layoutItems||!layoutItems.length) return []; if(autoFromLayout.length) return autoFromLayout; const centralThreshold=0.6; const centralCount=layoutItems.reduce((acc,li)=> acc + (Math.abs(li?.pos?.z ?? 0) < 0.5 ? 1:0),0); if(centralCount / layoutItems.length > centralThreshold) return []; let altCounter=0; return layoutItems.map(li=>{ const p=li.pos||{}; const r=li.rot||{}; const clone={...li}; const isCentralZ=Math.abs(p.z ?? 0)<0.5; if(isCentralZ){ const side = altCounter++ %2 ===0 ? 1: -1; clone.pos={...p,z:side*WALL_Z}; // Invertimos: lado + mira hacia -Z (Math.PI), lado - mira hacia +Z (0)
        clone.rot={...r,y:side===1?Math.PI:0}; } else if (Math.abs(p.z ??0) < WALL_Z*0.6){ const side=(p.z ??0)>=0?1:-1; clone.pos={...p,z:side*WALL_Z}; clone.rot={...r,y:side===1?Math.PI:0}; } else { const side=(p.z ??0)>0?1:-1; clone.rot={...r,y:side===1?Math.PI:0}; } if(!clone.pos) clone.pos={}; if(!clone.pos.y || Math.abs(clone.pos.y)<0.01) clone.pos.y=2.1; if(typeof clone.pos.x!=="number") clone.pos.x=0; return clone; }); },[layoutItems,autoFromLayout]);
  const slotPositions = useMemo(()=>{ if(!artworks||!artworks.length) return []; if(normalizedLayout.length>0) return []; const pairs=Math.ceil(artworks.length/2); if(pairs===0) return []; const span=Math.max(0.0001, lastX-firstX); const spacing = pairs>1? span/(pairs-1):0; const arr=[]; for(let i=0;i<artworks.length;i++){ const side=i%2===0?1:-1; const pairIndex=Math.floor(i/2); const x=firstX + pairIndex*spacing; const y=2.1; const z=side*WALL_Z; const rotY= side===1?Math.PI:0; arr.push({ position:[x,y,z], rotation:[0,rotY,0] }); } return arr; },[artworks,normalizedLayout,firstX,lastX]);
  return (<>
    ...
    {normalizedLayout && normalizedLayout.length>0 ? normalizedLayout.map((li,i)=> <Picture key={li.mural?.id||li.muralId||i} src={li.mural?.imagenUrlWebp||li.mural?.url_imagen} title={li.mural?.titulo||"Sin título"} artist={li.mural?.autor||"Desconocido"} year={li.mural?.anio||"N/A"} description={li.mural?.descripcion||"Sin descripción"} technique={li.mural?.tecnica||"No especificada"} dimensions={li.mural?.dimensiones||"Dimensiones no especificadas"} position={[li.pos?.x??0, li.pos?.y??2.1, li.pos?.z??0]} rotation={[li.rot?.x??0, li.rot?.y??0, li.rot?.z??0]} floorTextureUrl={salaTextures?.piso} onClick={()=>setSelectedArtwork(li.mural||null)} showPlaque={passedInitialWall && !selectedArtwork && !showList && !showCollection && !showInstructions} selected={selectedArtwork && selectedArtwork.id===li.mural?.id} selectedArtwork={selectedArtwork} spotlightIntensity={li.spotlightIntensity??0} frameStyle={li.frameStyle} />) : artworks.map((art,i)=>{ const sp=slotPositions[i] || { position:[0,1.5,0], rotation:[0,0,0] }; return <Picture key={art.id||i} {...art} position={sp.position} rotation={sp.rotation} floorTextureUrl={salaTextures?.piso} onClick={()=>setSelectedArtwork(art)} showPlaque={passedInitialWall && !selectedArtwork && !showList && !showCollection && !showInstructions} selected={selectedArtwork && selectedArtwork.id===art.id} selectedArtwork={selectedArtwork} />; })}
...
function ZoomModal({ artwork, onClose, onCollectionUpdate, userId }) { const modalRef=useRef(null); const [isInCollection,setIsInCollection]=useState(false); const [collectionMessage,setCollectionMessage]=useState(""); const [isUpdating,setIsUpdating]=useState(false); const [zoom,setZoom]=useState(1); const [pan,setPan]=useState({x:0,y:0}); const imageContainerRef=useRef(null); const dragState=useRef({active:false,startX:0,startY:0,baseX:0,baseY:0}); const containerSize=useRef({w:0,h:0}); const clampZoom=v=>Math.min(4,Math.max(0.5,v)); const handleWheel=useCallback(e=>{ e.preventDefault(); const factor=e.deltaY*0.0015; setZoom(z=>clampZoom(z - factor)); },[]); useEffect(()=>{ const el=imageContainerRef.current; if(!el) return; el.addEventListener("wheel",handleWheel,{passive:false}); return ()=>el.removeEventListener("wheel",handleWheel); },[handleWheel]); const zoomIn=()=>setZoom(z=>clampZoom(z*1.2)); const zoomOut=()=>setZoom(z=>clampZoom(z/1.2)); const resetZoom=()=>setZoom(1); const handleSlider=e=> setZoom(clampZoom(parseInt(e.target.value,10)/100)); useEffect(()=>{ setIsInCollection(isInPersonalCollection(artwork,onCollectionUpdate.collection)); },[artwork,onCollectionUpdate.collection]); useEffect(()=>{ const esc=e=>{ if(e.key==="Escape") onClose(); }; window.addEventListener("keydown",esc); return ()=>window.removeEventListener("keydown",esc); },[onClose]); const handleCollectionAction=useCallback(async(e)=>{ e.stopPropagation(); if(!userId){ setCollectionMessage("⚠️ Inicia sesión para guardar"); setTimeout(()=>setCollectionMessage(""),3000); return;} setIsUpdating(true); try{ const muralId=artwork.id; if(!muralId) throw new Error("Sin muralId"); if(isInCollection){ await fetch(`/api/usuarios/${userId}/collection`,{ method:"DELETE", headers:{"Content-Type":"application/json"}, body: JSON.stringify({ muralId })}); setCollectionMessage("🗑️ Removido de tu colección"); } else { await fetch(`/api/usuarios/${userId}/collection`,{ method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify({ muralId })}); setCollectionMessage("✅ ¡Añadido a tu colección!"); } onCollectionUpdate?.update && onCollectionUpdate.update(); } catch(err){ setCollectionMessage("❌ Error"); } finally { setIsUpdating(false); setTimeout(()=>setCollectionMessage(""),3000); } },[artwork,isInCollection,onCollectionUpdate,userId]); const updateBounds=useCallback(()=>{ const el=imageContainerRef.current; if(!el) return; containerSize.current={ w:el.clientWidth, h:el.clientHeight }; },[]); useEffect(()=>{ updateBounds(); window.addEventListener("resize",updateBounds); return ()=>window.removeEventListener("resize",updateBounds); },[updateBounds]); const clampPan=useCallback((x,y)=>{ if(zoom<=1) return {x:0,y:0}; const {w,h}=containerSize.current; const maxX=(w*(zoom-1))/2; const maxY=(h*(zoom-1))/2; return { x:Math.max(-maxX,Math.min(maxX,x)), y:Math.max(-maxY,Math.min(maxY,y)) }; },[zoom]); const onPointerDown=e=>{ if(zoom<=1) return; dragState.current={ active:true,startX:e.clientX,startY:e.clientY,baseX:pan.x,baseY:pan.y}; }; const onPointerMove=e=>{ if(!dragState.current.active) return; const dx=e.clientX - dragState.current.startX; const dy=e.clientY - dragState.current.startY; setPan(clampPan(dragState.current.baseX+dx, dragState.current.baseY+dy)); }; const endDrag=()=>{ dragState.current.active=false; }; useEffect(()=>{ window.addEventListener("pointermove",onPointerMove); window.addEventListener("pointerup",endDrag); window.addEventListener("pointerleave",endDrag); return ()=>{ window.removeEventListener("pointermove",onPointerMove); window.removeEventListener("pointerup",endDrag); window.removeEventListener("pointerleave",endDrag); }; },[onPointerMove]); useEffect(()=>{ if(zoom<=1) setPan({x:0,y:0}); },[zoom]); if(!artwork) return null; return <AnimatePresence><motion.div ref={modalRef} initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={onClose} className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4"><motion.div initial={{y:50,opacity:0}} animate={{y:0,opacity:1}} exit={{y:50,opacity:0}} transition={{type:"spring",stiffness:300,damping:30}} onClick={e=>e.stopPropagation()} className="relative bg-gradient-to-br from-slate-950/95 via-slate-900/90 to-slate-800/85 backdrop-blur-2xl border border-slate-700/60 text-slate-100 rounded-2xl shadow-[0_8px_40px_-8px_rgba(0,0,0,0.6)] overflow-hidden max-w-5xl w-full flex flex-col md:flex-row">
      <button onClick={onClose} className="absolute top-3 right-3 px-3 py-2 rounded-lg bg-slate-700/70 hover:bg-slate-600/80 text-sm" aria-label="Cerrar">✕</button>
      <div className="md:w-5/12 w-full p-6 flex flex-col justify-between gap-5">
        <div className="space-y-4">
          <h2 className="text-3xl font-bold leading-tight bg-gradient-to-r from-amber-300 via-amber-200 to-amber-400 text-transparent bg-clip-text">{artwork.title}</h2>
          ...
          <p className="text-slate-300/90 text-sm leading-relaxed max-h-48 overflow-y-auto pr-1">{artwork.description}</p>
        </div>
        <div className="space-y-4">
          <div className="flex items-center justify-between text-xs text-slate-400"><span>Zoom: {(zoom*100).toFixed(0)}% {zoom>1 && "(Rueda y arrastra)"}</span>{collectionMessage && <span className="text-amber-300 font-medium">{collectionMessage}</span>}</div>
          <input type="range" min={50} max={400} value={Math.round(zoom*100)} onChange={handleSlider} className="w-full accent-amber-400" />
          <div className="flex gap-2"><button onClick={zoomOut} className="flex-1 py-2 rounded-lg bg-slate-800/70 hover:bg-slate-700/80 text-sm">−</button><button onClick={resetZoom} className="flex-1 py-2 rounded-lg bg-slate-800/70 hover:bg-slate-700/80 text-sm">Reset</button><button onClick={zoomIn} className="flex-1 py-2 rounded-lg bg-slate-800/70 hover:bg-slate-700/80 text-sm">＋</button></div>
          <div className="flex gap-2">
            <button onClick={handleCollectionAction} disabled={isUpdating} className={`flex-1 py-3 px-4 rounded-lg font-semibold text-sm transition ${isUpdating?"bg-slate-600 cursor-not-allowed": isInCollection?"bg-red-600/80 hover:bg-red-600":"bg-amber-400 hover:bg-amber-500 text-black"}`}>{isUpdating?"Guardando...": isInCollection?"Quitar de favoritos":"Añadir a favoritos"}</button>
            <button onClick={()=> navigator?.clipboard?.writeText(window.location.href).catch(()=>{})} className="px-4 rounded-lg bg-slate-800/70 hover:bg-slate-700/80 text-sm">Compartir</button>
          </div>
        </div>
      </div>
      <div ref={imageContainerRef} className={`md:w-7/12 w-full relative bg-slate-950/70 overflow-hidden group rounded-l-2xl md:rounded-l-none md:rounded-r-2xl select-none ${zoom>1?"cursor-grab":"cursor-zoom-in"}`} onPointerDown={onPointerDown}>
        <div style={{transform:`translate(${pan.x}px,${pan.y}px) scale(${zoom})`, transition: dragState.current.active?"none":"transform 0.25s ease", transformOrigin:"center center"}} className="w-full h-full flex items-center justify-center p-4">
          <img src={artwork.src} alt={artwork.title} className="object-contain max-h-[80vh] pointer-events-none drop-shadow-[0_0_28px_rgba(255,200,120,0.22)]" draggable={false} />
        </div>
...
export default function GalleryRoom({ salaId=1, murales=[], layout=[], scene=null, texturaPared=null, texturaPiso=null, onRoomChange, availableRooms=[] }) {
  ...
  const hasUsableLayout = useMemo(()=>{ if(!layout||!layout.length) return false; return layout.some(l=>{ const p=l.pos||{}; const r=l.rot||{}; return (p.x&&Math.abs(p.x)>0.01)||(p.z&&Math.abs(p.z)>0.01)||(p.y&&Math.abs(p.y)>0.01)||(r.y&&Math.abs(r.y)>0.01)||(l.scale && l.scale!==1); }); },[layout]);
  const effectiveLayout = hasUsableLayout? layout: [];
  const validArtworks = useMemo(()=> murales.filter(art=> art && (art.url_imagen||art.imagenUrlWebp)).map(art=> ({ ...art, src: art.imagenUrlWebp || art.url_imagen, title: art.titulo||"Sin título", artist: art.autor||"Desconocido", year: art.anio||"N/A", description: art.descripcion||"Sin descripción", technique: art.tecnica||"No especificada", dimensions: art.dimensiones||"Dimensiones no especificadas" })), [murales]);
  const galleryDimensions = useMemo(()=>{ if(effectiveLayout.length>0){ const xs=effectiveLayout.map(l=> l.pos?.x ?? 0); const minX=Math.min(...xs,0); const maxX=Math.max(...xs,0); const padding=6; const dynamicLength=Math.max(GALLERY_CONFIG.HALL_LENGTH, maxX-minX + padding*2); const center=(minX+maxX)/2; return { dynamicLength, dynamicCenterX:center, firstX:minX - padding, lastX:maxX + padding, contentLength: maxX-minX + padding*2, wallMarginInitial:4, wallMarginFinal:3 }; } return calculateGalleryDimensions(validArtworks); },[effectiveLayout,validArtworks]);
  const artworkPositions = useMemo(()=> effectiveLayout.length>0? [] : calculateArtworkPositions(validArtworks,galleryDimensions.firstX,GALLERY_CONFIG.PICTURE_SPACING,galleryDimensions.contentLength), [effectiveLayout,validArtworks,galleryDimensions]);
  const handleSelectArtwork=art=> setSelectedArtwork(art); const handleCloseModal=()=> setSelectedArtwork(null);
  useEffect(()=>{ if(selectedArtwork){ setFocusArtwork(selectedArtwork); setFocusTrigger(v=>v+1); } },[selectedArtwork]);
  const handleSelectRoom=room=>{ if(room?.id){ window.location.href=`/museo/sala/${room.id}`; return; } if(onRoomChange) onRoomChange(room); setShowEndModal(false); };
  ...
  return (<><div className="gallery-container absolute top-0 left-0 w-full h-full bg-black">
    ...
    <AnimatePresence>{selectedArtwork && <ZoomModal artwork={selectedArtwork} onClose={handleCloseModal} userId={userId} onCollectionUpdate={{ collection:personalCollection, update:fetchCollection }} />}</AnimatePresence>
    <EndOfHallModal open={showEndModal} onClose={handleCloseEndModal} rooms={otherRooms} onSelectRoom={handleSelectRoom} onExit={handleExit} />
    {showQuickList && <div className="absolute top-4 left-4 z-50 bg-slate-900/85 backdrop-blur border border-slate-700 rounded-xl p-4 max-h-[70vh] overflow-y-auto w-64 text-sm space-y-2">
      <div className="flex justify-between items-center mb-2"><span className="font-semibold text-amber-300">Obras</span><button className="text-xs text-slate-400 hover:text-amber-300" onClick={()=>setShowQuickList(false)}>Cerrar</button></div>
      {validArtworks.map(a=> <button key={a.id} onClick={()=>{ setFocusArtwork(a); setFocusTrigger(v=>v+1); }} className={`block w-full text-left px-2 py-1 rounded hover:bg-slate-800/60 ${focusArtwork?.id===a.id? 'bg-slate-800/80 text-amber-300':''}`}>{a.title}</button>)}
      <div className="text-[10px] text-slate-500 pt-1">Atajo: L lista</div>
    </div>}
  </div></>);
}
